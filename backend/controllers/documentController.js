...
// const extractTitleFromText = (text) => {
//   const regex = /\bLR\d{6}\b/i; // Matches LR followed by exactly 6 digits
//   const match = text.match(regex);
//   return match ? match[0].toUpperCase() : null;
// };
...
//     if (!titleNumber) {
//       return res.status(400).json({ message: "Title number not found in document." });
//     }

//     // Check if title number exists in the database
//     const result = await pool.query("SELECT * FROM land_records WHERE title_number = $1", [titleNumber]);
...
//     res.status(201).json({
//       message: landFound ? "Land found!" : "Land not found!",
//       titleNumber,
//       file: uploadedFile,
//     });
//   } catch (error) {
